...
export function createChoicePrompt(
  ...
  return [
    ...
    "- Resolve where you end up, who sees you, and what is left behind through natural action.",
    ...
    "- Use PLAYER_CONTEXT.loadout when resolving actions that depended on gear.",
    "- If a weapon or tool was used, show one clear consequence of that use.",
    "- End with a small future-facing hook or memory, not a cliffhanger that invalidates the conclusion.",
    "",
    "Outcome guidance:",
    `- Resolve only because the player has made ${minimumJobStoryChoiceDepth}, 4, or ${maximumJobStoryChoiceDepth} choices on this job path.`,
    ...
    JSON.stringify({
      ...
      narration: "Four clear sentences resolving the full path naturally.",
